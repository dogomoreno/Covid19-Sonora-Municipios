...
casosacumdia <- filter(Casos,Fecha==as.Date("2020-07-04"))
...
Decesos <- read_csv("Decesosdiarios.csv", 
     col_types = cols(DECESOS = col_integer(), 
        ...
        MUNICIPIO = col_character(), NUEVOS = col_integer()), 
    locale = locale(encoding = "ISO-8859-1"))
decesosacumdia <- filter(Decesos,Fecha==as.Date("2020-07-04"))
...
```{r}
Casossem <- group_by(Casos, MUNICIPIO, Semana = isoweek(Fecha)) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS))
Casos_Semanales <- ggplot(subset(Casossem, MUNICIPIO %in% c("Hermosillo", "Cajeme", "Nogales", "San Luis Río Colorado", "Navojoa", "Guaymas", "Caborca", "Agua Prieta"))) +
    geom_col(mapping = aes(x = Semana, y = `CASOS SEMANALES`, fill = `CASOS SEMANALES`, label= ACUMULADOS)) +
    scale_fill_gradient(low = "#58BCBC", high = "black", limits = c(1,1500)) + 
    ...
    theme(axis.line = element_line(linetype = "solid"),
          ...
          legend.position = "none") +labs(y = "Casos semanales", x = NULL, title  = "Casos semanales") + 
  theme(panel.grid.major = element_line(colour = "white"), panel.grid.minor = element_line(colour = "white"),                                   panel.background = element_rect(fill = "gray95"),legend.position = "none") +labs(y = "Casos diarios", fill = NULL)
ggplotly(Casos_Semanales, tooltip = c("y", "label"))
saveWidget(ggplotly(Casos_Semanales, tooltip = c("y", "label")), file = "Casossem4.html")
```
