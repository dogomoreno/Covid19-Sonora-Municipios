...
Fechasem <- paste0("Al reporte del ", day(Sys.Date()), " de ", months.Date(Sys.Date())," de ", year(Sys.Date())," | Confirmados acumulados de ",weekdays(Sys.Date())," a ", weekdays(Sys.Date()-1)," (por fecha de corte)")
Fechadom <- paste0("Al reporte del ", day(Sys.Date()), " de ", months.Date(Sys.Date())," de ", year(Sys.Date()),"  | Cifras al ", weekdays(Sys.Date()-1)," de cada semana (por fecha de corte).")
...
ggsave("Semanales/s02.png",Estatus , width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
ggsave("Semanales/s03.png",Casossemson, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
ggsave("Semanales/s09.png",Decesossemson, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
Hospsemson <- ggplot(Hospsemana) +
  ...
  temasinejes +
  labs(y = NULL, 
       x = NULL,legend= NULL, title  = "<span style = 'font-size:14pt'>Covid-19 en Sonora:</span><br><span style = 'color:#F79646';>Hospitalizados al cierre de semana</span>", 
       subtitle= Fechadom, caption =fuente)
 Hospsemson

ggsave("Semanales/s04.png",Hospsemson, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
ggsave("Semanales/s05.png",Activos, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
ggsave("Semanales/s11.png",Pruebassemson, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
Positividad <- ggplot() +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"), (max(as.Date(Sonora.DF$Fecha)) + 60)), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = "%", 
       ...
       subtitle= Fechahoy, caption =fuente)
Positividad
ggsave("Semanales/s12.png",Positividad, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
Casosmuni <- ggplot(Casossemana) +
  ...
  geom_dl( data = subset(Casossemana , Fecha == max(Fecha)), aes(x=Fecha, y= casos.resto,  label = paste0("Otros municipios\n", casos.resto, sep="")), color="#F79646", 
           method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 0.6, fontfamily= "Lato Black")) +
  geom_dl( data = subset(Casossemana , Fecha == max(Fecha)), aes(x=Fecha, y= casos.hmo, label = paste0("Hermosillo\n", casos.hmo, sep="")), color="#01A2AC", 
           method = list(dl.trans(x = x + 0.2), "last.bumpup", cex = 0.6, fontfamily= "Lato Black")) +
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"),  (max(as.Date(Sonora.DF$Fecha)) + 82)), date_breaks = "1 month", date_labels = "%B") +
  ...
  labs(y = "Casos confirmados", 
       ...
       subtitle= Fechasem, caption =fuente)
Casosmuni
ggsave("Semanales/s06.png",Casosmuni, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
Decesosmuni <- ggplot(Casossemana) +
  ...
  labs(y = "Decesos confirmados", 
       ...
       subtitle= Fechasem, caption =fuente)
Decesosmuni
ggsave("Semanales/s14.png",Decesosmuni, width = 5 * (16/9), height = 5, type = "cairo", dpi = 300)
...
Indicadores <- ggplot() +
  geom_hline(yintercept=100, linetype="dashed", color = "#CE3F41", alpha=0.8) +
  geom_text(aes(x = as.Date("2020-04-15"), y = (102),
                label = "Alta"), stat = "unique", family = "Lato Black",
            size = 2, color = "#CE3F41",  hjust=0, alpha=0.8) +
  geom_hline(yintercept=50, linetype="dashed", color = "#FFA17B", alpha=0.8) +
  geom_text(aes(x = as.Date("2020-04-15"), y = (52),
                label = "Substancial"), stat = "unique", family = "Lato Black",
            size = 2, color = "#FFA17B",  hjust=0, alpha=0.8) +
  geom_hline(yintercept=10, linetype="dashed", color = "#FECF7D", alpha=0.8) +
  geom_text(aes(x = as.Date("2020-04-15"), y = (12),
