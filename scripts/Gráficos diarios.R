...
Gravgraf <- ggplot(Sonora.Hosp) +
  ...
  labs(y = "Casos diarios\n(promedio móvil 7 días, log)", 
       ...
       subtitle= Fechahoy, caption =fuente) 
ggsave("Gráficos diarios/diariohospitalizados.png",Gravgraf, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Mapa_incidencia
ggsave("Gráficos diarios/diariomapinci.png",Mapa_incidencia, width = 5/2 * (16/9), height = 5, type = "cairo", dpi = 400)
...
PruebasSon <- ggplot(Sonora.DF) +
  ...
  scale_y_continuous(expand=c(0,0), limits=c(0, max(Sonora.DF$Pruebas.diarias,na.rm=TRUE)+100)) +
...
ggsave("Gráficos diarios/decesosdiasem.png",Decesosdiasem, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
ggsave("Gráficos diarios/casosdiasem.png",Casosdiasem, width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
...
Hospitalizados <- Sonora.DF %>% ggplot(aes(x= Fecha, y= Hospitalizados)) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-08-01"), max(as.Date(Sonora.DF$Fecha))+50), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = NULL, 
...
Activos <- Sonora.DF %>% ggplot() +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-06-19"), max(as.Date(Sonora.DF$Fecha))+50), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes + 
  theme(legend.text = element_text(family = "Lato", size = 7), legend.background = element_rect(fill="transparent"),
...
Pruebasdiasem <- Pruebasemana %>% ggplot(aes(x= diasemana, y= Pruebas)) +
  ...
  labs(y = NULL, 
       ...
       subtitle= Fechahoy, caption =fuente)  
Pruebasdiasem
ggsave("Gráficos diarios/pruebasdiasem.png",Pruebasdiasem,  width = 5 * (16/9), height = 5, type = "cairo", dpi = 400)
