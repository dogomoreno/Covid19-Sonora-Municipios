...
Casosmesson <- ggplot(Casosmes) +
  ...
  geom_text( data = subset(Casosmes, Fecha<as.Date("2020-04-30")), aes(x=Fecha, y= casos.mensuales, label= scales::comma(casos.mensuales)), family="Lato Black", size= 4, color="black", angle=90, hjust = -0.2) +
  geom_text( data = subset(Casosmes,Fecha>as.Date("2020-04-30")), aes(x=Fecha, y= casos.mensuales, label= scales::comma(casos.mensuales)), family="Lato Black", size= 4, color="white", angle=90, hjust = 1.1) +
...
Decesosmesson <- ggplot(Decesosmes) +
  ...
  geom_text( data = subset(Decesosmes, Fecha<as.Date("2020-04-30")), aes(x=Fecha, y= decesos.mensuales, label= decesos.mensuales), family="Lato Black", size= 4, color="black", angle=90, hjust = -0.2) +
  geom_text( data = subset(Decesosmes,Fecha>as.Date("2020-04-30")), aes(x=Fecha, y= decesos.mensuales, label= decesos.mensuales), family="Lato Black", size= 4, color="white", angle=90, hjust = 1.2) +
...
ggsave("Gráficos diarios/Mensual.png",p3, width = 5 * (16/9), height = 10, type = "cairo", dpi = 300)
...
temames <-  theme(axis.line.y = element_line(linetype = "solid"), axis.line.x = element_blank(),
                  ...
                  plot.title = element_markdown(family = "Lato Black", size = 18),  
                  plot.subtitle = element_text(family = "Lato Light", size = 10, color = "black"), legend.title = element_blank(),
                  axis.text.y = element_text(family = "Lato", size =8),   panel.grid= element_blank(),
...
Casosmesson <- ggplot(Casosmes) +
  geom_col(aes(x=Fecha, y= casos.mensuales, fill= casos.mensuales), color= "#005156", size=0.3, width=20) +
  scale_fill_gradient2(low = "#DEF2F2", mid= "#01A2AC", high = "#005156", midpoint = 5000) +
  geom_text( data = subset(Casosmes, Fecha<as.Date("2020-04-30")), aes(x=Fecha, y= casos.mensuales, label= scales::comma(casos.mensuales)), family="Lato Black", size= 4, color="black",  hjust = -0.2) +
  geom_text( data = subset(Casosmes,Fecha>as.Date("2020-04-30")), aes(x=Fecha, y= casos.mensuales, label= scales::comma(casos.mensuales)), family="Lato Black", size= 4, color="white", hjust = 1.1) +
  # geom_text(aes(x = as.Date("2020-04-30"), y = 8000,
  #            label = "69,936 casos acumulados"), stat = "unique", family = "Lato Black",
  #            size = 5, color = "black", hjust =1) +
  scale_x_date(expand=c(0,0), date_breaks = "1 month", date_labels = "%B\n%y") +
...
Decesosmesson <- ggplot(Casosmes) +
  geom_col(aes(x=Fecha, y= decesos.mensuales, fill= decesos.mensuales), color= "#4D1933", size=0.3, width=20) +
  scale_fill_gradient2(low = "#F0D1E0", mid= "#993366", high = "#4D1933", midpoint = 600) +
  geom_text( data = subset(Casosmes, Fecha<as.Date("2020-04-30")), aes(x=Fecha, y= decesos.mensuales, label= decesos.mensuales), family="Lato Black", size= 4, color="black", hjust = -0.2) +
  geom_text( data = subset(Casosmes,Fecha>as.Date("2020-04-30")), aes(x=Fecha, y= decesos.mensuales, label= decesos.mensuales), family="Lato Black", size= 4, color="white", hjust = 1.2) +
  # geom_text(aes(x = as.Date("2020-03-30"), y = 800,
  #               label = "5,991 decesos acumulados"), stat = "unique", family = "Lato Black",
  #           size = 5, color = "black", hjust =1) +
  scale_x_date(expand=c(0,0), date_breaks = "1 month", date_labels = "%B\n%y") +
...
p3
ggsave("Gráficos diarios/Mensual_flip.png",p3, width = 5 * (16/9), height = 10, type = "cairo", dpi = 300)
