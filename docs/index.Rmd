...
output: 
  flexdashboard::flex_dashboard:
    navbar:
      - { title: "05/09/2021", href: "http://www.luisarmandomoreno.com/", align: right }
    theme: flatly
    orientation: rows
---
<style>                     
...
```{r, include=FALSE}
POBMUN <- read_csv("C:/Users/luism/OneDrive/R/COVID/Municipios/Bases/POBMUN.csv", locale = locale(encoding = "ISO-8859-1"), col_types = cols(CVEGEO = col_character()))
Sonora.DF <- read_csv("C:/Users/luism/OneDrive/R/COVID/Municipios/Bases/ST_SonoraInformesCOVID.csv", 
...
```{r, include=FALSE}
Casos <- read_csv("C:/Users/luism/OneDrive/R/COVID/Municipios/Bases/Casosdiarios.csv", 
...
```{r, include=FALSE}
Decesos <- read_csv("C:/Users/luism/OneDrive/R/COVID/Municipios/Bases/Decesosdiarios.csv", 
...
write.csv(casos_ult_dias,'C:/Users/luism/OneDrive/R/COVID/Municipios/ResultadoCSV/Municipiosdiasdesdecasos.csv')
```
...
DecesosSon <- ggplot(Sonora.DF) +
  ...
  geom_point(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 1.8, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#73264D")) +
      ...
      scale_x_date(expand=c(0,0), limits=c(as.Date("2020-03-16"), (Sys.Date()+5))) +
       theme_minimal() +
    theme(
    legend.position = "none",
     panel.background = element_rect(fill= "white"),
...
mapacovid <- leaflet(capa_munison) %>% 
  ...
  addLegend(position = "topright", pal = incipal, values = ~capa_munison$MS, opacity=1, group= "INCIDENCIA", 
              labFormat = function(type, cuts, p) {  
               paste0(labs)} ,
...
Casos_Semanales <- ggplot(subset(Casossem, MUNICIPIO %in% c("Hermosillo", "Cajeme", "Nogales", "San Luis Río Colorado", "Navojoa", "Guaymas", "Caborca", "Agua Prieta", "Huatabampo", "Puerto Peñasco", "Etchojoa", "Empalme", "Cananea","Magdalena", "Álamos", "Bácum"))) +
  ...
  theme(panel.grid.major = element_line(colour = "white"), panel.grid.minor = element_line(colour = "white"),                                   panel.background = element_rect(fill = "white"),legend.position = "none")
