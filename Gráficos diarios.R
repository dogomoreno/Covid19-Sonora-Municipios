...
Fechahoy <- "Al reporte del 13 de julio de 2021"
...
subtitulo <- "Casos confirmados en los últimos 8 días por 100 mil habitantes\nAl reporte del 13/07/2021"
...
Letalidad <- Sonora.DF %>% ggplot(aes(x= Fecha, y= Letalidad)) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"), as.Date("2021-08-20")), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
...
Hospitalizados <- Sonora.DF %>% ggplot(aes(x= Fecha, y= Hospitalizados)) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-08-01"), as.Date("2021-08-15")), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = NULL, 
...
Activos <- Sonora.DF %>% ggplot(aes(x= Fecha, y= Ambulatorios.Activos)) +
  ...
  scale_x_date(expand=c(0,0), limits = c(as.Date("2020-06-19"), as.Date("2021-08-20")), date_breaks = "1 month", date_labels = "%B") +
  coord_cartesian(expand = FALSE, clip = 'off') +
  theme_bw() +
  temaejes +
  labs(y = NULL, 
