...
DecesosSon <- ggplot(Sonora.DF) +
  ...
  geom_point(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 1.8, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#73264D")) +
      ...
      scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"), as.Date("2021-01-23"))) +
       theme_minimal() +
    theme(
    legend.position = "none",
     panel.background = element_rect(fill= "white"),
...
Row
-----------------------------------------------------------------------
### **Incidencia y mortalidad municipal al 18/01/2021**
...
mutate(IS=if_else(INCIDENCIASEM>108,5, if_else(INCIDENCIASEM>60,4, if_else(INCIDENCIASEM>32,3,if_else(INCIDENCIASEM>16,2,1))))) %>% filter(Fecha==max(as.Date(Fecha))) 
...
mutate(MS=if_else(MORTALIDADSEM>18,5, if_else(MORTALIDADSEM>10,4, if_else(MORTALIDADSEM>5,3,if_else(MORTALIDADSEM>2,2,1))))) %>% filter(Fecha==max(as.Date(Fecha)))
...
Indicadores
=====================================
**Al 18 de enero de 2021**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
