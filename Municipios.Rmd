...
LetalidadSS <- ggplot(Sonora.SS) +
   geom_col(aes(x= Letalidad, y= SS, color= "Pacientes curados"), fill= "#E0A3C1", color= "black", size= 0.3)+
          theme_minimal() +
    theme(
...
### **Casos acumulados al 21 de agosto de 2020**
...
### **Días desde el último caso confirmado (corte al 21/09/2020)**  {data-width=500}
...
### **Decesos acumulados al 21 de septiembre de 2020** {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=39)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=39)
...
Indicadores
=====================================
**Al 21 de septiembre de 2020**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
