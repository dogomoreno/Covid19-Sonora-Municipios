...
casosacumdia <- filter(Casos,Fecha==as.Date("2020-06-30"))
...
Decesos <- read_csv("Decesosdiarios.csv", 
     col_types = cols(DECESOS = col_integer(), 
        ...
        MUNICIPIO = col_character(), NUEVOS = col_integer()), 
    locale = locale(encoding = "ISO-8859-1"))
decesosacumdia <- filter(Decesos,Fecha==as.Date("2020-06-30"))
...
Mapa_casos <- ggplot(capa_munison_casos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low = "#58BCBC", high = "black", limits = c(1,2900), breaks = c(1, 500, 1000, 1500, 2000, 2500))+
    theme_void() +
...
Column
-----------------------------------------------------------------------
### Casos acumulados al 30 de junio de 2020 {data-width=500}
...
Column
-----------------------------------------------------------------------
### Decesos acumulados al 30 de junio de 2020 {data-width=500}
...
Indicadores
=====================================
**Al 30/06/2020** [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
