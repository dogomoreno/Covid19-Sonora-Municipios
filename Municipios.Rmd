...
casosacumdia <- filter(Casos,Fecha==as.Date("2020-06-28"))
...
Decesos <- read_csv("Decesosdiarios.csv", 
     col_types = cols(DECESOS = col_integer(), 
        ...
        MUNICIPIO = col_character(), NUEVOS = col_integer()), 
    locale = locale(encoding = "ISO-8859-1"))
decesosacumdia <- filter(Decesos,Fecha==as.Date("2020-06-28"))
...
Mapa_casos <- ggplot(capa_munison_casos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low = "#58BCBC", high = "black", limits = c(1,2700), breaks = c(1, 400, 800, 1200, 1600, 2000, 2400))+
    theme_void() +
...
Column
-----------------------------------------------------------------------
### Casos acumulados al 28 de junio de 2020 {data-width=500}
...
Mapa_decesos <- ggplot(capa_munison_decesos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low= "#993366", high = "black", limits = c(1,240), breaks = c(1, 40, 80, 120, 160, 200, 240))+
    theme_void() +
   theme(plot.title = (element_text(family = "Lato Black", size = 15, color = "#01A2AC")),
    legend.position = "left",
    legend.key.height = unit (2, "cm"), axis.text = element_blank(),
...
Column
-----------------------------------------------------------------------
### Decesos acumulados al 28 de junio de 2020 {data-width=500}
...
Tabla
=====================================
Al 28/06/2020 (Incidencia = Casos por cada 100 mil habs., Mortalidad = Decesos por cada 100 mil habs., Letalidad = Decesos por cada 100 casos)
```{r}
...
CDSELECT<-select(Indicadores, "MUNICIPIO","POBLACION", "CASOS","CASOS NUEVOS", "DECESOS","DECESOS NUEVOS","INCIDENCIA","MORTALIDAD", "LETALIDAD")
...
reactable(CDSELECT1,searchable = TRUE, highlight = TRUE, defaultSorted = "CASOS", defaultSortOrder = "desc",
  defaultPageSize = 15, minRows = 15,
   ...
   columns = list(
    ...
    ), INCIDENCIA = colDef(format = colFormat(digits = 1)), MORTALIDAD = colDef(format = colFormat(digits = 1)), LETALIDAD = colDef(format = colFormat(digits = 1))),
