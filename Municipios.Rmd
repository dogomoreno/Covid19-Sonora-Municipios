...
### **Casos acumulados al 08 de noviembre de 2020**
...
### **Días desde el último caso confirmado (corte al 08/11/2020)**  {data-width=500}
...
### **Decesos acumulados al 08 de noviembre de 2020** {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=46)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=46)
