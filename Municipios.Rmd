...
casosacumdia <- filter(Casos,Fecha==as.Date("2020-07-01"))
...
Decesos <- read_csv("Decesosdiarios.csv", 
     col_types = cols(DECESOS = col_integer(), 
        ...
        MUNICIPIO = col_character(), NUEVOS = col_integer()), 
    locale = locale(encoding = "ISO-8859-1"))
decesosacumdia <- filter(Decesos,Fecha==as.Date("2020-07-01"))
...
Mapa_casos <- ggplot(capa_munison_casos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low = "#58BCBC", high = "black", limits = c(1,3000), breaks = c(1, 500, 1000, 1500, 2000, 2500, 3000))+
    theme_void() +
...
Column
-----------------------------------------------------------------------
### Casos acumulados al 01 de julio de 2020 {data-width=500}
...
Mapa_decesos <- ggplot(capa_munison_decesos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low= "#993366", high = "black", limits = c(1,260), breaks = c(1, 50, 100, 150, 200, 250))+
    theme_void() +
   theme(plot.title = (element_text(family = "Lato Black", size = 15, color = "#01A2AC")),
    legend.position = "left",
    legend.key.height = unit (2, "cm"), axis.text = element_blank(),
...
Column
-----------------------------------------------------------------------
### Decesos acumulados al 01 de julio de 2020 {data-width=500}
...
Indicadores
=====================================
**Al 1 de julio de 2020** [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
