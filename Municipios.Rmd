...
LetalidadSS <- ggplot(Sonora.SS) +
   geom_col(aes(x= Letalidad, y= SS, color= "Pacientes curados"), fill= "#EFD1E0", color= "#73264D")+
          theme_minimal() +
    theme(
...
### **Casos acumulados al 13 de agosto de 2020**
...
### **Días desde el último caso confirmado (corte al 13/08/2020)**  {data-width=500}
...
### **Decesos acumulados al 13 de agosto de 2020** {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=33)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=33)
...
Indicadores
=====================================
**Al 13 de agosto de 2020**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
