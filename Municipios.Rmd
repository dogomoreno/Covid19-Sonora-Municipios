...
RecuperadosSonora = function(...) return(11056)
...
valueBox(CasosSon, icon = "fa-certificate", color = "#01787E")
```
...
valueBox(DecesosSon, icon= "fa-plus", color = "#993366")
```
...
CasosSon <- ggplot(Sonora.DF) +
  geom_col(aes(x= Fecha, y= Casos.diarios, fill= "Casos diarios"), color = "white", size = 0.1) +
   geom_line(aes(x= Fecha, y= Casos.media.7d, color= "Promedio móvil 7d"), linetype= "solid", size=1)+
   scale_fill_manual(name="", values= c("Casos diarios" = "#58bcbc")) + 
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#005155")) +
...
DecesosSon <- ggplot(Sonora.DF) +
  geom_col(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 0.1) +
   geom_line(aes(x= Fecha, y= Decesos.media.7d, color= "Promedio móvil 7d"), linetype= "solid", size=1)+
   scale_fill_manual(name="", values= c("Decesos diarios" = "#E0A3C1")) + 
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#4D1A33")) +
      scale_y_continuous(expand = c(0, 0)) +
      scale_x_date(expand=c(0,0)) +
       theme_minimal() +
    theme(
...
### **Pacientes curados vs Decesos acumulados**
```{r, include=FALSE}
RecupSon <- ggplot(Sonora.DF) +
   geom_line(aes(x= Fecha, y= Pacientes_Curados, color= "Pacientes curados" ), linetype= "solid", size=1)+
   geom_line(aes(x= Fecha, y= Decesos, color= "Decesos" ), linetype= "solid", size=1)+
      scale_color_manual(name="", values= c("Pacientes curados" = "#366092", "Decesos" = "#73264D" ))+
...
```{r}
ggplotly(RecupSon, tooltip = c("x", "y")) %>%
  layout(legend = list(orientation = "h",
                   y = 80, x = 0))
```
...
Mapa_casos <- ggplot(capa_munison_casos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low = "#58BCBC", high = "black", limits = c(1,6500), breaks = c(1, 1000, 2000, 3000, 4000, 5000, 6000))+
    theme_void() +
...
### **Casos acumulados al 20 de julio de 2020**
...
### **Días desde el último caso confirmado (corte al 20/07/2020)**  {data-width=500}
...
Mapa_decesos <- ggplot(capa_munison_decesos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low= "#993366", high = "black", limits = c(1,410), breaks = c(1, 50, 100, 150, 200, 250,300,350,400))+
    theme_void() +
   theme(plot.title = (element_text(family = "Lato Black", size = 15, color = "#01A2AC")),
    legend.position = "left",
    legend.key.height = unit (2, "cm"), axis.text = element_blank(),
...
### **Decesos acumulados al 20 de julio de 2020** {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana != 30)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana != 30)
...
Decesos_Todos <- ggplot(data = Decesos) +
    ...
    axis.title = element_text(family = "Lato", size = 8)) +
    labs(y = NULL, x = NULL, title  = NULL,  fill = NULL, caption ="Elaboración propia con información de la Secretaría de Salud del Estado de Sonora")
ggplotly(Decesos_Todos)
```

Indicadores
=====================================
**Al 19 de julio de 2020**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
