...
### **Casos acumulados al 04 de octubre de 2020**
...
### **Días desde el último caso confirmado (corte al 04/10/2020)**  {data-width=500}
...
### **Decesos acumulados al 04 de octubre de 2020* {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=41)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=41)
