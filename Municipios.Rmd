...
DecesosSon <- ggplot(Sonora.DF) +
  ...
  geom_point(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 1.8, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#73264D")) +
      ...
      scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"), as.Date("2021-01-10"))) +
       theme_minimal() +
    theme(
    legend.position = "none",
     panel.background = element_rect(fill= "white"),
...
Row
-----------------------------------------------------------------------
### **Incidencia y mortalidad municipal al 07/01/2021 (capas semanales al domingo más reciente)**

```{r include=FALSE}
Casossemana <- Casos %>% mutate(w = isoweek(Fecha)) %>% group_by(w) %>% mutate (Reporte=max(as.Date(Fecha))) %>%  
  mutate(Semana = paste(year(Reporte), isoweek(Reporte), sep="-"))
...
casossempob   <- mutate(casossempob , IS=if_else(INCIDENCIASEM>162,5, if_else(INCIDENCIASEM>59,4, if_else(INCIDENCIASEM>30,3,if_else(INCIDENCIASEM>15,2,1)))))
Casossemanales <- casossempob %>% select(CVEGEO, MUNICIPIO, 'CASOS SEMANALES', INCIDENCIASEM, IS)

Decesossemana <- Decesos %>% mutate(w = isoweek(Fecha)) %>% group_by(w) %>% mutate (Reporte=max(as.Date(Fecha))) %>%  
  mutate(Semana = paste(year(Reporte), isoweek(Reporte), sep="-"))
...
Decesossempob  <- Decesossempob %>% filter(Semana=="2021-53")
...
```{r}
Casossemana <- Casos %>% mutate(w = isoweek(Fecha)) %>% group_by(w) %>% mutate (Reporte=max(as.Date(Fecha)))  %>%  
  mutate(Semana = paste(year(Reporte), isoweek(Reporte), sep="-")) %>% filter(Semana!="2021-1")
...
Indicadores
=====================================
**Al 07 de enero de 2021** (casos semanales al domingo más reciente)  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
