...
casosacumdia <- filter(Casos,Fecha==as.Date("2020-06-27"))
...
Decesos <- read_csv("Decesosdiarios.csv", 
     col_types = cols(DECESOS = col_integer(), 
        ...
        MUNICIPIO = col_character(), NUEVOS = col_integer()), 
    locale = locale(encoding = "ISO-8859-1"))
decesosacumdia <- filter(Decesos,Fecha==as.Date("2020-06-27"))
...
Column
-----------------------------------------------------------------------
### Casos acumulados al 27 de junio de 2020 {data-width=500}
...
Mapa_decesos <- ggplot(capa_munison_decesos, aes(map_id = id)) +
    ...
    scale_fill_gradient(low= "#993366", high = "black", limits = c(1,230), breaks = c(1, 40, 80, 120, 160,200))+
    theme_void() +
   theme(plot.title = (element_text(family = "Lato Black", size = 15, color = "#01A2AC")),
    legend.position = "left",
    legend.key.height = unit (2, "cm"), axis.text = element_blank(),
...
Column
-----------------------------------------------------------------------
### Decesos acumulados al 27 de junio de 2020 {data-width=500}
