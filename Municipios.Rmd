...
Row
-----------------------------------------------------------------------
### **Incidencia y mortalidad municipal al 06/12/2020 (capas semanales al domingo más reciente**
...
Decesossempob  <- Decesossempob %>% filter(Semana==49)
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=50)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=50)
...
Indicadores
=====================================
**Al 06 de diciembre de 2020** (casos semanales al domingo más reciente)  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
