...
LetalidadSS <- ggplot(Sonora.SS) +
   geom_col(aes(x= Letalidad, y= SS, color= "Pacientes curados"), fill= "#73264D", color= "black", size= 0.5)+
          theme_minimal() +
    theme(
...
### **Casos acumulados al 17 de agosto de 2020**
...
### **Días desde el último caso confirmado (corte al 17/08/2020)**  {data-width=500}
...
### **Decesos acumulados al 17 de agosto de 2020** {data-width=500}
...
Casossem <- group_by(Casossemana, MUNICIPIO, Reporte, Semana) %>% summarise('CASOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(CASOS)) %>% filter(Semana!=34)
...
Decesossem <- group_by(Decesossemana, MUNICIPIO, Reporte, Semana) %>% summarise('DECESOS SEMANALES' = sum(NUEVOS), ACUMULADOS=max(DECESOS)) %>% filter(Semana!=34)
...
Decesos_Todos <- ggplot(data = Decesos) +
      geom_tile(mapping = aes(x = Fecha, y = MUNICIPIO, fill = NUEVOS, text = paste(
               "ACUMULADOS: ", DECESOS, "\n")), color = "white") +
     scale_fill_gradient(low= "#993366", high = "black", limits = c(0,50), breaks = c(0, 10, 20, 30, 40, 50) ) +
     scale_x_date(date_breaks = "weeks" , date_labels = "%d-%m")+
     theme_minimal() +
...
Indicadores
=====================================
**Al 17 de agosto de 2020**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
