...
DecesosSon <- ggplot(Sonora.DF) +
  ...
  geom_point(aes(x= Fecha, y= Decesos.diarios, fill= "Decesos diarios"), color = "white", size = 1.8, stroke=0.5, alpha=0.6) +
   ...
   scale_color_manual(name="", values= c("Promedio móvil 7d" = "#73264D")) +
      ...
      scale_x_date(expand=c(0,0), limits = c(as.Date("2020-04-01"), as.Date("2021-01-24"))) +
       theme_minimal() +
    theme(
    legend.position = "none",
     panel.background = element_rect(fill= "white"),
...
Row
-----------------------------------------------------------------------
### **Incidencia y mortalidad municipal al 21/01/2021**
...
casossempob <- Casossemana %>% 
  mutate(IS=if_else(INCIDENCIASEM>(round(quantile(Casossemana$INCIDENCIASEM, 0.90, na.rm=TRUE),0)),5, 
                    if_else(INCIDENCIASEM>(round(quantile(Casossemana$INCIDENCIASEM, 0.75, na.rm=TRUE),0)),4, 
                            if_else(INCIDENCIASEM>(round(quantile(Casossemana$INCIDENCIASEM, 0.50, na.rm=TRUE),0)),3,
                                    if_else(INCIDENCIASEM>(round(quantile(Casossemana$INCIDENCIASEM, 0.25, na.rm=TRUE),0)),2,1))))) %>% filter(Fecha==max(as.Date(Fecha))) 
...
mutate(MS=if_else(MORTALIDADSEM>(round(quantile(Decesossemana$MORTALIDADSEM, 0.90, na.rm=TRUE),0)),5, 
                    if_else(MORTALIDADSEM>(round(quantile(Decesossemana$MORTALIDADSEM, 0.75, na.rm=TRUE),0)),4, 
                            if_else(MORTALIDADSEM>(round(quantile(Decesossemana$MORTALIDADSEM, 0.50, na.rm=TRUE),0)),3,
                                    if_else(MORTALIDADSEM>(round(quantile(Decesossemana$MORTALIDADSEM, 0.25, na.rm=TRUE),0)),2,1))))) %>% filter(Fecha==max(as.Date(Fecha)))
...
Mapa <- Indicadores %>%  left_join(diaspmap, by = c("CVEGEO","MUNICIPIO")) %>% 
  ...
  mutate(IA=if_else(INCIDENCIAACUM>1678,5, if_else(INCIDENCIAACUM>1239,4,if_else(INCIDENCIAACUM>708,3,if_else(INCIDENCIAACUM>452,2,1)))))%>% 
  mutate(MA=if_else(MORTALIDADACUM>184,5, if_else(MORTALIDADACUM>163,4, if_else(MORTALIDADACUM>110,3,if_else(MORTALIDADACUM>80,2,1)))))
...
incipal <-  colorFactor(c("#cf597e", "#ed9c72", "#e9e29c", "#71be83", "#009392"), levels= c("5","4","3","2","1"), na.color ="#e8e6e6")
mortpal <-  colorFactor(c("#cf597e", "#ed9c72", "#e9e29c", "#71be83", "#009392"), levels= c("5","4","3","2","1"), na.color ="#e8e6e6")
...
mapacovid <- leaflet(capa_munison) %>% 
    ...
    addLayersControl( 
      baseGroups = c("INCIDENCIA ÚLTIMA SEMANA", "INCIDENCIA ACUMULADA", "MORTALIDAD ÚLTIMA SEMANA", "MORTALIDAD ACUMULADA"), 
  ...
  addLegend(position = "bottomleft", pal = incipal, values = ~capa_munison$IA, opacity=1, group= "INCIDENCIA", 
              labFormat = function(type, cuts, p) {  
               paste0(labs)},
              title = "CLASIFICACIÓN", na.label = "N/A") 
...
Indicadores
=====================================
**Al 21 de enero de 2021**  [Descarga esta tabla](https://onedrive.live.com/download.aspx?resid=5ADDF6870413EAC9!32159&authkey=!AC27R9r4fs_DWP4)
```{r}
